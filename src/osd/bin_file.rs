...
impl TileKind {
	...
	/// Finds the tile kind matching the provided bin file size in bytes
	///
	/// # Errors
	/// Returns `InvalidSizeError` if no matching tile kind is found
...
pub enum SeekError {
	...
	OutOfBoundsError { file_path: PathBuf, new_pos: isize },
}
...
impl LoadError {
	pub fn tile_kind_mismatch<P: AsRef<Path>>(file_path: P, loaded: TileKind, requested: TileKind) -> Self {
		Self::LoadedTileKindDoesNotMatchRequested {
			file_path: file_path.as_ref().to_path_buf(),
			loaded,
			requested,
		}
...
impl BinFileReader {
	/// Opens a bin file for reading
	///
	/// # Errors
	/// Returns `OpenError` if opening fails or the file size is invalid
	pub fn open<P: AsRef<Path>>(path: P) -> Result<Self, OpenError> {
		let file = File::open(&path)?;
		let tile_kind = tile::Kind::for_bin_file_size_bytes(file.metadata()?.len()).map_err(|error| {
			let InvalidSizeError(size) = error;
			OpenError::invalid_size(&path, size)
		})?;
		log::info!(
			"detected {} kind of tiles in {}",
			tile_kind,
	...
	/// Reads a tile from the current position
	///
	/// # Errors
	/// Returns `IOError` if reading fails
	///
	/// # Panics
	/// Panics if the read bytes do not correspond to a valid tile of the expected kind
	pub fn read_tile(&mut self) -> Result<Tile, IOError> {
		Ok(Tile::try_from(self.read_tile_bytes()?).unwrap())
	}

	/// Seeks to the specified position and reads a tile
	///
	/// # Errors
	/// see `seek` and `read_tile` functions
	///
	/// # Panics
	/// see `read_tile` function
	...
	///
	/// # Errors
	/// Returns `SeekError` if seeking fails or new position is out of bounds
	...
	///
	/// # Errors
	/// Returns `SeekError` if seeking fails
	...
	/// Reads all tiles and returns them as a `TileGrid`
	///
	/// # Errors
	/// Returns `SeekReadError` if reading fails
	pub fn into_tile_grid(self) -> Result<TileGrid, SeekReadError> {
		Ok(self.read_tiles()?.into_tile_grid())
	}

	/// Reads all tiles and returns them as a `Vec<Tile>`
	///
	/// # Errors
	/// Returns `IOError` if reading fails
...
/// Loads a tile collection from the provided bin file
///
/// # Errors
...
/// Loads the specified font part from the provided directory using normalized file names
///
/// # Errors
/// Returns `LoadError` if loading fails
///
/// # Panics
/// Panics if the loaded tile kind does not match a valid tile kind
pub fn load_norm<P: AsRef<Path>>(
	dir: P,
	tile_kind: TileKind,
	ident: Option<&str>,
	part: FontPart,
...
/// Loads both font parts from the provided paths
///
/// # Errors
/// Returns `LoadError` if loading fails
///
/// # Panics
/// Panics if the loaded tile kinds do not match the expected tile kinds
pub fn load_extended<P: AsRef<Path>>(base_path: P, ext_path: P) -> Result<Vec<Tile>, LoadError> {
	let base_tiles = load(&base_path)?;
	let base_tile_kind = base_tiles
		.tile_kind()
		.expect("should not fail for collections from bin files");
	let ext_tiles = load(&ext_path)?;
	let ext_tile_kind = ext_tiles
		.tile_kind()
		.expect("should not fail for collections from bin files");
...
/// Loads both font parts from the provided paths and checks that they match the requested tile kind
///
/// # Errors
/// Returns `LoadError` if loading fails
pub fn load_extended_check_kind<P: AsRef<Path>>(
	base_path: P,
	ext_path: P,
	requested_tile_kind: TileKind,
) -> Result<Vec<Tile>, LoadError> {
	fn check_tile_kind<P: AsRef<Path>>(
		file_path: P,
		tiles: &[Tile],
		expected_tile_kind: TileKind,
	) -> Result<(), LoadError> {
		let tile_kind = tiles
			.tile_kind()
			.expect("should not fail for collections from bin files");
...
pub fn normalized_file_path<P: AsRef<Path>>(
	dir: P,
	tile_kind: TileKind,
	ident: Option<&str>,
	part: FontPart,
) -> PathBuf {
	[dir.as_ref().to_path_buf(), normalized_file_name(tile_kind, ident, part)]
		.into_iter()
		.collect()
}

/// Loads the base font part from the provided directory using normalized file names
///
/// # Errors
/// Returns `LoadError` if loading fails
pub fn load_base_norm<P: AsRef<Path>>(
	dir: P,
	tile_kind: TileKind,
	ident: Option<&str>,
) -> Result<Vec<Tile>, LoadError> {
...
/// Loads both font parts from the provided directory using normalized file names
///
/// # Errors
/// Returns `LoadError` if loading fails
pub fn load_extended_norm<P: AsRef<Path>>(
	dir: P,
	tile_kind: TileKind,
	ident: Option<&str>,
) -> Result<Vec<Tile>, LoadError> {
...
impl TileSet {
	/// Loads the tile set from the provided paths
	///
	/// # Errors
	/// Returns `LoadError` if loading fails
	pub fn load_bin_files<P: AsRef<Path>>(
		sd_path: P,
		sd_2_path: P,
		hd_path: P,
		hd_2_path: P,
	...
	/// Loads the tile set from the provided directory using normalized file names
	///
	/// # Errors
	/// Returns `LoadError` if loading fails
...
impl BinFileWriter {
	/// Creates a new bin file for writing
	///
	/// # Errors
	/// Returns `IOError` if creation fails
	...
	/// Writes a tile to the bin file
	///
	/// # Errors
	/// Returns `TileWriteError` if writing fails
	pub fn write_tile(&mut self, tile: &Tile) -> Result<(), TileWriteError> {
		...
		match self.tile_kind {
			Some(tile_kind) => {
				if tile_kind != tile.kind() {
					return Err(TileWriteError::TileKindMismatchError {
						written_kind: tile_kind,
						writing_kind: tile.kind(),
					});
				}
			},
	...
	/// Finishes writing the bin file
	///
	/// # Errors
	/// Returns `TileWriteError` if less than 256 tiles have been written
