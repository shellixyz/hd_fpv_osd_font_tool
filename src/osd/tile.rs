...
impl Kind {
	...
	/// Finds the tile kind matching the provided size in bytes
	///
	/// # Errors
	...
	/// Finds the tile kind matching the provided height
	///
	/// # Errors
	/// Returns `InvalidHeightError` if no matching tile kind is found
...
pub enum LoadError {
	...
	FileError { file_path: PathBuf, error: IOError },
	#[error(transparent)]
	ReadError(IOError),
	#[error(transparent)]
...
impl Tile {
	...
	/// Loads a tile from the provided image file
	///
	/// # Errors
	...
	pub fn load_image_file<P: AsRef<Path>>(path: P) -> Result<Self, LoadError> {
		...
		let kind = Kind::try_from(Dimensions::from(image.dimensions())).map_err(|error| {
	...
	/// Reads a tile from the provided binary file reader
	///
	/// # Errors
	/// Returns `LoadError` if reading fails
	///
	/// # Panics
	/// Panics if the read bytes do not match the expected size for a tile
...
impl TryFrom<Bytes> for Tile {
	...
	fn try_from(bytes: Bytes) -> Result<Self, Self::Error> {
		...
		Ok(Self {
			kind,
			image: ImageBuffer::from_raw(kind.dimensions().width(), kind.dimensions().height(), bytes).unwrap(),
		})
	}
}
