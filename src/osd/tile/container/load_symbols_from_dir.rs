...
pub enum LoadSymbolsFromDirError {
	...
	SymbolSpanDoesNotMatchName { file_name: PathBuf, real_span: usize },
...
enum SymbolDirFileType {
	Tile { index: usize },
	Symbol { start_index: usize, end_index: usize },
}
...
fn identify_file_name<P: AsRef<Path>>(path: P) -> Option<SymbolDirFileType> {
	...
	if let Some(captures) = FILE_NAME_RE.captures(path.as_ref().file_name().unwrap().to_string_lossy().as_ref()) {
		let start_index = captures
			...
			.as_str()
			.parse()
			.expect("failed to parse start index");
...
/// Loads symbols from the provided directory
///
/// # Errors
/// Returns `LoadSymbolsFromDirError` if loading fails
#[allow(clippy::too_many_lines)]
pub fn load_symbols_from_dir<P: AsRef<Path>>(
	dir_path: P,
	max_symbols: usize,
) -> Result<Vec<Symbol>, LoadSymbolsFromDirError> {
	let mut symbol_files = BTreeMap::new();
	let dir_files_iter =
		dir_files_iter(&dir_path).map_err(|error| LoadSymbolsFromDirError::dir_list_files(&dir_path, error))?;
		...
		if let Some(file_type) = identify_file_name(&file_path) {
			...
			match symbol_files.entry(file_type.start_index()) {
				btree_map::Entry::Vacant(entry) => {
					entry.insert((file_path, file_type));
				},
				btree_map::Entry::Occupied(entry) => {
					let (existing_path, _) = entry.get();
					return Err(LoadSymbolsFromDirError::OverlappingSymbolFiles(
						file_path,
						existing_path.clone(),
					));
				},
		...
		let symbol = match symbol_files.get(&tile_index) {
			Some((file_path, file_type)) => {
				if file_type.start_index() < tile_index {
					let Some(previous_symbol_file_path) = previous_symbol_file_path else {
						unreachable!();
					};
					return Err(LoadSymbolsFromDirError::OverlappingSymbolFiles(
						previous_symbol_file_path.clone(),
						file_path.clone(),
					));
				}
				...
				match Symbol::load_image_file(file_path) {
					Ok(loaded_symbol) => {
						if loaded_symbol.span() != file_type.span() {
							return Err(LoadSymbolsFromDirError::SymbolSpanDoesNotMatchName {
								file_name: file_path.clone(),
								real_span: loaded_symbol.span(),
							});
						}
					...
					Err(error) => match &error {
						SymbolLoadError::ImageReadError(image_error) => {
							use crate::image::ReadError as IRE;
							match image_error {
								IRE::OpenError {
									file_path: _,
									error: image_error,
								} => match image_error.kind() {
									std::io::ErrorKind::NotFound => None,
									_ => return Err(error.into()),
								},
								IRE::DecodeError { .. } => return Err(error.into()),
							}
						},
						_ => return Err(error.into()),
					},
	...
	let symbols = match tile_kind {
		Some(tile_kind) => {
			let Some(last_some_index) = symbols.iter().rposition(Option::is_some) else {
				unreachable!();
			};
			symbols[0..=last_some_index]
				.iter()
				.map(|symbol| symbol.clone().unwrap_or_else(|| Symbol::new(tile_kind)))
				.collect()
		},
