...
/// Loads tiles from the provided directory
///
/// # Errors
/// Returns `LoadTilesFromDirError` if loading fails
pub fn load_tiles_from_dir<P: AsRef<Path>>(path: P, max_tiles: usize) -> Result<Vec<Tile>, LoadTilesFromDirError> {
	...
	for index in 0..max_tiles {
		let tile_path: PathBuf = [path.as_ref(), Path::new(&format!("{index:03}.png"))].iter().collect();
		let tile = match Tile::load_image_file(tile_path) {
			Ok(loaded_tile) => Some(loaded_tile),
			Err(error) => match &error {
				TileLoadError::ImageReadError(ImageReadError::OpenError {
					file_path: _,
					error: open_error,
	...
	let tiles = match tile_kind {
		Some(tile_kind) => {
			let Some(last_some_index) = tiles.iter().rposition(Option::is_some) else {
				unreachable!();
			};
			tiles[0..=last_some_index]
				.iter()
				.map(|tile| tile.clone().unwrap_or_else(|| Tile::new(tile_kind)))
				.collect()
		},
