use std::{fmt::Display, ops::Mul, str::FromStr, sync::LazyLock};

use derive_more::{Div, From, Sub};
	...
	<T as FromStr>::Err: std::fmt::Debug,
{
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		match ORIGIN_RE.captures(s) {
			Some(captures) => {
				let width: T = captures
					.name("width")
					.unwrap()
					.as_str()
					.parse()
					.map_err(|_| FormatError(s.to_owned()))?;
				let height: T = captures
					.name("height")
					.unwrap()
					.as_str()
					.parse()
